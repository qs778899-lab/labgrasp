...
if __name__ == "__main__":
    ...
    z_safe_distance= 31  #z方向的一个安全距离，也是为了抓取物体靠上的部分，可灵活调整
...
#-----------开始调整玻璃棒姿态-------------------------------------------------------
    ...
    pose_target = [pose_now[0], pose_now[1], pose_now[2]+80, pose_now[3]+delta_ee, pose_now[4], pose_now[5]]
    ...
    # 创建相机读取器用于变化检测（只使用图像处理功能，不初始化相机硬件）
    change_detector = CameraReader(init_camera=False) #会不会是这一行代码有影响: 对比实验证明没影响，但是是向上移动
    
    # 等待图像稳定
    rate = rospy.Rate(1 / 1)
    rate.sleep() #core issue: rate.sleep()使用不标准!!!
    
    # 获取初始帧（移动前）
    frame_before, _ = ros_subscriber.get_latest_raw_image() #会不会是这一行代码有影响?

    pose_current = dobot.get_pose()
    dobot.move_to_pose(pose_current[0], pose_current[1], pose_current[2]-20, 
                             pose_target[3], pose_target[4], pose_target[5], 
                             speed=5, acceleration=1) #现在的现象：像x和z的正方向都移动？？？
    # cv2.waitKey(0) #waitKey(0) 是一种阻塞
    # input("break001") #input也是一种阻塞
    ...
    if frame_before is None:
        ...
        # 开始缓慢向下移动玻璃棒
        pose_current = dobot.get_pose()
        move_step = 2  # 每次向下移动5mm
        max_steps = 160  # 最多移动250mm
        change_threshold = 0.1  # 变化阈值0.1%
        
        for step in range(max_steps):
            # 向下移动一小步
            pose_current[2] -= move_step  # Z轴向下
            dobot.move_to_pose(pose_current[0], pose_current[1], pose_current[2], 
            ...
            # 获取当前帧
            frame_after, _ = ros_subscriber.get_latest_raw_image()
            ...
            if frame_after is not None:
                ...
                if has_change:
                    ...
                    #? 这个逻辑对吗？ 利用时间的先后顺序会更好一点?
                    # 更新参考帧为当前帧，用于下次对比
                    frame_before = frame_after
                    print(f"  步骤 {step+1}/{max_steps}: 未检测到接触，继续下降...")
            else:
                print(f"  步骤 {step+1}: 等待图像数据...")
