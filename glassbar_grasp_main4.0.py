...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:
            ...
            #mark: 获取ROS原始图像数据并检测方向（非阻塞）
            raw_image, img_timestamp = ros_subscriber.get_latest_raw_image()
            # has_new_image = raw_image is not None and (
            ...
            if has_new_image:
                ...
                if detected_angles:
                    ...
                    if last_valid_detected_angles is not None:
                        ...
                        avg_angle = last_valid_avg_angle  # 使用上次的平均角度
                        print(f"使用上次检测结果: {detected_angles}, 平均: {avg_angle:.2f}°")
                    else:
                        avg_angle = 0.0  # 从未检测到，默认0
            else:
                # 没有新图像
                if last_valid_detected_angles is not None:
                    # 使用上次有效的检测结果
                    detected_angles = last_valid_detected_angles
                    avg_angle = last_valid_avg_angle  # 使用上次的平均角度
                    print(f"当前无新图像，使用上次检测到的角度: {detected_angles}, 平均: {avg_angle:.2f}°")
                else:
